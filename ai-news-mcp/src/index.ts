...
class AINewsServer {
  ...
  private async fetchAINews(args: any) {
    ...
    result.forEach((news, index) => {
      ...
      if (news.author) report += `- **作者**: ${news.author}\n`;
      report += `- **链接**: ${news.link}\n`;
      if (news.summary) report += `\n${news.summary}...\n`;
      report += `\n---\n\n`;
    });
  ...
  private async fetchArxivPapers(args: any) {
  ...
  private async generateDailyReport(args: any) {
  ...
  private async generateWeeklyReport(args: any) {
